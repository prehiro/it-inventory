...
import { receiveAction, repairRestoreAction, type ActionResult, type RepairRestoreResult } from "@/app/actions/inventory";
...
export function ReceiveForm({ models }: { models: { id: string; type: string; model: string; brand: string; category: string }[] }) {
  ...
  return (
    <>
        ...
        <div>
          <div className="mb-1.5 flex items-center gap-2">
            ...
            {repairInfo && !repairDone && (
              <span
                style={{ ["--neon" as string]: "rgba(245,158,11,0.55)" }}
                className="inline-flex animate-text-glow-breath items-center gap-1 text-xs font-medium text-amber-600 dark:text-amber-400"
              >
                <span
                  style={{ ["--neon" as string]: "rgba(245,158,11,0.55)" }}
                  className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-amber-50 text-amber-600 animate-status-breath dark:bg-amber-500/15 dark:text-amber-400"
                >
            ...
            <div className="animate-fade-in">
              ...
              <div className="mt-6 flex gap-3">
                <button
                  ...
                  className="flex-1 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
                >
                  No
                </button>
                <button
                  onClick={handleRepairYes}
                  disabled={repairRestoring}
                  className="flex-1 rounded-lg bg-amber-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-amber-500 disabled:opacity-60"
                >
      ...
      {/* REPAIR DONE CONFIRMATION */}
      {repairDone && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4 backdrop-blur-sm dark:bg-black/50">
          <div
            onClick={() => setRepairDone(false)}
            role="dialog"
            aria-modal="true"
            className="w-full max-w-sm animate-fade-in rounded-2xl bg-white p-6 text-center shadow-xl ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-700"
          >
            <div className="animate-fade-in">
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Item is now <span className="font-medium text-emerald-600 dark:text-emerald-400">AVAILABLE</span> and ready to release.
              </p>
