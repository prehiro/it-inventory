...
export function Topbar({ name, role }: { name: string; role: string }) {
  ...
  const isPcLedger = pathname === "/pc-ledger";

  // Inner container width: min(1152px, available) normally; full content area on PC Ledger.
  // Using px→px max-width so the CSS transition interpolates smoothly (spring bounce).
  const [areaWidth, setAreaWidth] = useState(1152);
  useEffect(() => {
    function measure() {
      const header = document.querySelector("header");
      if (!header) return;
      setAreaWidth(header.clientWidth - 48); // px-6 both sides
    }
    measure();
  ...
  const innerMaxWidth = isPcLedger ? areaWidth : Math.min(1152, areaWidth);
  const innerStyle = {
      ...
      <header className="sticky top-0 z-20 flex h-16 items-center border-b border-slate-200 bg-white/90 backdrop-blur-lg dark:border-slate-800 dark:bg-slate-900/90">
        <div className="flex w-full px-6">
          ...
          >
            {/* ── Search ── */}
            <form
              key={isPcLedger ? "search-wide" : "search-centered"}
              onSubmit={onSearch}
              className={`relative w-full max-w-xs ${
                isPcLedger ? "animate-topbar-bounce-left" : "animate-topbar-bounce-right"
              }`}
            >
              <svg
            ...
            <div
              key={isPcLedger ? "actions-wide" : "actions-centered"}
              className={`ml-auto flex items-center gap-1 ${
                isPcLedger ? "animate-topbar-bounce-right" : "animate-topbar-bounce-left"
              }`}
            >
              <TopbarLowStock />
